import 'bootstrap/dist/css/bootstrap.min.css'

export default function REZERVACIJE() {
    <>
        <h1>Rezervacije termina</h1>
        <p>
            Rezervacije termina za individualne  treninge
            možete izvršiti klikom na : <button>Rezerviraj termin</button>  ili osobnim dolaskom
            na adresu studija : BluGym Centar Osijek,Pavla Pejačevića 22.

            Ukoliko želite rezervirati termin za grupni trening,
            to možete izvršiti osobnim dolaskom u studio,kako bih utvrdili detalje.


            Ako su treninzi u tijeku nažalost, nećemo moći odgovoriti na Vaše upite stoga u ponudi
            ne postoji kontakt forma za komuniciranje putem e-maila,
            društvenih mreža ili telefonskog poziva, ali radimo na tome.

        </p>

    </>

}